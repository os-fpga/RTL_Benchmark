...
if {$pre_cts} {
	...
	# clock period. This affects the following timing path in 1:1 master
	# mode:
	# SPI_CLOCK~($spi_m_sclk_delay)~>SPI_M_SCLK_ORG~>spi_in_r_reg*/CP (clock path)
...
if {$slave_support} {
	# The master-mode clock will also propagate to a_signal_sync1's D.
	# However, a_signal_sync1 is only used to sample the slave-mode clock.
	set_false_path -from [get_clocks SPI_M_SCLK_*] -to [get_cells u_spi_spiif/spi_clk_in_syn/a_signal_sync1*]
	...
	set_false_path -hold -to [get_cells u_spi_spiif/spi_clk_in_syn/a_signal_sync1*]
}
