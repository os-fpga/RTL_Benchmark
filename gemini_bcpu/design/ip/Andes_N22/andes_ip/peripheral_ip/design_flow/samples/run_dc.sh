...
if [ ! -e "ip_database/${NDS_PLATFORM}_cpu_subsystem.ddc" ]; then
   cd $CORE_SYN_PATH
   ...
   cp     $CORE_SYN_PATH/ddc/${NDS_PLATFORM}_cpu_subsystem$itr.ddc $SCRIPT_PATH/ip_database/${NDS_PLATFORM}_cpu_subsystem.ddc
   cd $SCRIPT_PATH
fi
...
for ip in $ip_list; do
	...
	if [ ! -e "ip_database/$ip.ddc" ]; then
		...
		# Synthesis by DC
		dc_shell-t -wait 1440 -f $SCRIPT_PATH/syn_dc.tcl | tee dc.log
