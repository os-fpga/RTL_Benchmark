...
//! This Latency sequence, after a bench reset, generates the following transactions:
...
//! - Send 1 single word WR transaction, 1 single word RD transaction.
class anvu_virtual_latency_seq extends uvm_sequence;
	`uvm_object_utils(anvu_virtual_latency_seq)
	`uvm_declare_p_sequencer(anvu_virtual_sequencer)

	bit restrictedInitFlowId[int];
	bit restrictedTargFlowId[int];

	function new(string name = "anvu_virtual_latency_seq");
		super.new(name);
	endfunction
	task body();
		anvu_top_env   top;
		...
		`uvm_info("anvu_test","Starting Latency",uvm_pkg::UVM_LOW)
		...
		foreach ( nocMaxAddrByInitFlowId[flowId] ) begin
			...
			foreach ( connections[k] ) begin
				...
				foreach(opcodes[l]) begin
					...
					,	connections[k].targFlow.str()
					,	(opcodes[l]==anvu_noc_definitions_pkg::READ?"RD":"WR")
					),uvm_pkg::UVM_LOW);
					case (nocSocketTypeByFlowId[flow.id()])
						anvu_commons_pkg::AXI: begin
							anvu_axi_master_latency_seq seq = anvu_axi_master_latency_seq::type_id::create();
						...
						anvu_commons_pkg::APB: begin
							anvu_apb_master_latency_seq seq = anvu_apb_master_latency_seq::type_id::create();
							seq.startAddr    = connections[k].initStartAddr;
							seq.endAddr      = connections[k].initEndAddr;
							seq.opcode       = opcodes[l];
							seq.securityBase = securityBase;
						...
						anvu_commons_pkg::AHB: begin
							anvu_ahb_master_latency_seq seq = anvu_ahb_master_latency_seq::type_id::create();
							seq.startAddr    = connections[k].initStartAddr;
							seq.endAddr      = connections[k].initEndAddr;
							seq.opcode       = opcodes[l];
							seq.securityBase = securityBase;
		...
		`uvm_info("anvu_test","End of Latency",uvm_pkg::UVM_LOW)
		#(5*nocSlowestClkPeriod);  // Gives time for monitors to handle latest response if they have some internal delays.
	endtask
endclass


//! The goal of this test is to measure the latency for each memory map connections between NoC initiators and masters, one at a time.
...
class anvu_latency_test extends uvm_test;
	`uvm_component_utils(anvu_latency_test)

	//! top uvm_env which instantiates all sub uvm_env and the instrumented NoC
	anvu_top_env top;

	function new(string name = "anvu_latency_test", uvm_component parent);
	...
	task run_phase(uvm_phase phase);
		...
		anvu_virtual_latency_seq virtSeq = anvu_virtual_latency_seq::type_id::create();
