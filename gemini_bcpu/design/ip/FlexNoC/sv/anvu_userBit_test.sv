...
class anvu_virtual_userBit_seq extends uvm_sequence;
	`uvm_object_utils(anvu_virtual_userBit_seq)
	`uvm_declare_p_sequencer(anvu_virtual_sequencer)
	
	bit restrictedInitFlowId[int];
	bit restrictedTargFlowId[int];
	
	function new(string name = "anvu_virtual_userBit_seq");
		super.new(name);
	endfunction
	task body();
		anvu_top_env   top;
		...
		`uvm_info("anvu_test","Starting UserBit",uvm_pkg::UVM_LOW)
		...
		foreach ( nocMaxAddrByInitFlowId[flowId] ) begin
			...
			foreach ( connections[k] ) begin
				...
				||	connections[k].targFlow.isInternal()
				) continue;
				...
				case (nocSocketTypeByFlowId[flow.id()])
					anvu_commons_pkg::AXI: begin
						anvu_axi_master_userBit_seq seq = anvu_axi_master_userBit_seq::type_id::create();
					...
					anvu_commons_pkg::APB: begin
						anvu_apb_master_userBit_seq seq = anvu_apb_master_userBit_seq::type_id::create();
						`uvm_info("anvu_test","Connexion not tested as APB initiators don't drive user bits",uvm_pkg::UVM_LOW)
						seq.startAddr = connections[k].initStartAddr;
						seq.endAddr   = connections[k].initEndAddr;
						seq.start(p_sequencer.apbSequencer[nocApbInitIdxByFlowId[flowId]]);
					end
					anvu_commons_pkg::AHB: begin
						anvu_ahb_master_userBit_seq seq = anvu_ahb_master_userBit_seq::type_id::create();
						seq.startAddr = connections[k].initStartAddr;
						seq.endAddr   = connections[k].initEndAddr;
						seq.flow      = flow;
		...
		`uvm_info("anvu_test","End of UserBit",uvm_pkg::UVM_LOW)
		#(5*nocSlowestClkPeriod);  // Gives time for monitors to handle latest response if they have some internal delays.
	endtask
endclass


//! The goal of this test is to iterate over all user bit values
...
class anvu_userBit_test extends uvm_test;
	`uvm_component_utils(anvu_userBit_test)

	//! top uvm_env which instantiates all sub uvm_env and the instrumented NoC
	anvu_top_env top;

	function new(string name = "anvu_userBit_test", uvm_component parent);
	...
	task run_phase(uvm_phase phase);
		...
		anvu_virtual_userBit_seq virtSeq = anvu_virtual_userBit_seq::type_id::create();
