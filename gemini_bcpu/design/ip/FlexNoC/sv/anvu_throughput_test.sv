...
//! This Throughput sequence, after a bench reset, generates the following transactions:
...
//! - Send the necessary WR transaction so that at least 128 word transfers are performed.
//! - Send the necessary RD transaction so that at least 128 word transfers are performed.
class anvu_virtual_throughput_seq extends uvm_sequence;
	`uvm_object_utils(anvu_virtual_throughput_seq)
	`uvm_declare_p_sequencer(anvu_virtual_sequencer)
	
	bit restrictedInitFlowId[int];
	bit restrictedTargFlowId[int];

	function new(string name = "anvu_virtual_throughput_seq");
		super.new(name);
	endfunction
	task body();
		anvu_top_env   top;
		...
		`uvm_info("anvu_test","Starting Throughput",uvm_pkg::UVM_LOW)
				...
				foreach(opcodes[l]) begin
					...
					case (nocSocketTypeByFlowId[flow.id()])
						anvu_commons_pkg::AXI: begin
							anvu_axi_master_throughput_seq seq = anvu_axi_master_throughput_seq::type_id::create();
						...
						anvu_commons_pkg::APB: begin
							anvu_apb_master_throughput_seq seq = anvu_apb_master_throughput_seq::type_id::create();
							seq.startAddr    = connections[k].initStartAddr;
							seq.endAddr      = connections[k].initEndAddr;
							seq.opcode       = opcodes[l];
							seq.flow         = flow;
						...
						anvu_commons_pkg::AHB: begin
							anvu_ahb_master_throughput_seq seq = anvu_ahb_master_throughput_seq::type_id::create();
							seq.startAddr    = connections[k].initStartAddr;
							seq.endAddr      = connections[k].initEndAddr;
							seq.opcode       = opcodes[l];
							seq.flow         = flow;
		...
		`uvm_info("anvu_test","End of Throughput",uvm_pkg::UVM_LOW)
		#(5*nocSlowestClkPeriod);  // Gives time for monitors to handle latest response if they have some internal delays.
	endtask
endclass


//! The goal of this test is to measure the throughput for each memory map connections between NoC initiators and masters, one at a time. 
...
class anvu_throughput_test extends uvm_test;
	`uvm_component_utils(anvu_throughput_test)

	//! top uvm_env which instantiates all sub uvm_env and the instrumented NoC
	anvu_top_env top;

	function new(string name = "anvu_throughput_test", uvm_component parent);
	...
	function void end_of_elaboration();
		...
		// Set callbacks for slaves behaviour. 
		// Default callBacks used allow to configure the response depending on rspCfg objects
	endfunction

	//! Run the test by starting the virtual sequence.
	task run_phase(uvm_phase phase);
		bit initFlowId[int],targFlowId[int];
		anvu_virtual_throughput_seq virtSeq = anvu_virtual_throughput_seq::type_id::create();
