...
//------------------------------------------------------------------------------
//
//   Filename:           cdn_sd1000_t16ffc_01_vc176_2xa1xd3xe2y2r_behav_model.sv
...
//
//------------------------------------------------------------------------------
//   Description: This is behaviuor Verilog code for USB2 hard modules
//------------------------------------------------------------------------------


module clk_gating_cell_cdn_sd1000_t16ffc_01_vc176_2xa1xd3xe2y2r
#( 
   parameter
      NUM_SYNC_FLOPS      = 4,
      NUM_OFF_DELAY_FLOPS = 0,
      NUM_ACK_FLOPS       = 2,
      GATE_RESET_VALUE    = 1'b0
)(   
   input  reset_n,
   input  en,
   input  scanen,
   ...
   generate 
      ...
      begin
         data_sync_cdn_sd1000_t16ffc_01_vc176_2xa1xd3xe2y2r #(
            .NUM_FLOPS   (NUM_SYNC_FLOPS),
            .RESET_VALUE (GATE_RESET_VALUE)
         ) u_data_sync_en_sync (
            .clock       (clk_in),
            .reset_n     (reset_n),
            .d_in        (en), 
   ...
   endgenerate
   generate 
      if (NUM_OFF_DELAY_FLOPS == 0)
      begin
         assign en_delay_off = en_sync;
      end
      else
      begin
         data_delay_cdn_sd1000_t16ffc_01_vc176_2xa1xd3xe2y2r #( 
            .NUM_FLOPS     (NUM_OFF_DELAY_FLOPS)
         ) u_data_delay_delay_off (   
            .clock         (clk_in),
            .reset_n       (reset_n),
            .delay_en      ({NUM_OFF_DELAY_FLOPS{1'b1}}),
            .delay_half_en (1'b0),
            .d_in          (en_sync),
            .d_out         (en_delay_off)
         );
   ...
   endgenerate
   assign en_synced = en_sync || en_delay_off;
      assign en_synced_int = en_synced || scanen ;
      always @(clk_in or en_synced_int)
      begin
         if (!clk_in)
            enable_latch <= en_synced_int;
      end
      assign clk_out = clk_in && enable_latch ;
   generate 
      if (NUM_ACK_FLOPS == 0)
      begin
         assign en_ack = en_synced;
      end
      else
      begin
         data_delay_cdn_sd1000_t16ffc_01_vc176_2xa1xd3xe2y2r #( 
            .NUM_FLOPS     (NUM_ACK_FLOPS),
            .RESET_VALUE   (GATE_RESET_VALUE)
         ) u_data_delay_en_ack (   
            .clock         (clk_in),
            .reset_n       (reset_n),
            .delay_en      ({NUM_ACK_FLOPS{1'b1}}),
            .delay_half_en (1'b0),
            .d_in          (en_synced),
            .d_out         (en_ack)
         );
...
module clock_mux_cdn_sd1000_t16ffc_01_vc176_2xa1xd3xe2y2r 
#(
   parameter
      NUM_CLOCKS = 2,     
      NUM_CLOCK_SEL = 1   
...
module data_sync_cdn_sd1000_t16ffc_01_vc176_2xa1xd3xe2y2r 
#( 
   parameter
      RESET_VALUE = 1'b0,
      NUM_FLOPS   = 2
      ...
      generate 
         ...
         begin
            ...
            reg                 meta_select;
               ...
               begin
                  meta_prev_value <= d_in;
               end
               always @(negedge reset_n or posedge clock) 
               begin
                  if (!reset_n) 
                  begin
                     meta_sync_flop <= RESET_VALUE;
                  end
                  else 
                  begin
                     meta_sync_flop <= d_in;
            ...
            begin
               ...
               begin
                  data_sync_flop[NUM_FLOPS-1:1] <= data_sync_flop[NUM_FLOPS-2:0];
                     if (d_in === meta_prev_value)
                        data_sync_flop[0] <= d_in;
                     else if (data_sync_flop[0] !== meta_sync_flop)
                        data_sync_flop[0] <= meta_sync_flop;
                     else
                        data_sync_flop[0] <= meta_select ? d_in : meta_sync_flop;
               end
            end
            assign d_out_int = data_sync_flop[NUM_FLOPS-1];
         end
      endgenerate
   generate
      if (NUM_FLOPS == 0)
...
module reset_sync_cdn_sd1000_t16ffc_01_vc176_2xa1xd3xe2y2r 
#( 
   parameter
      NUM_FLOPS = 2
)(   
   ...
   wire   reset_n_synced;
      generate 
         if (NUM_FLOPS == 0)
         begin
            assign reset_n_synced = upstream_reset_in_n;
         end
         else
         begin
            ...
            reg                 meta_sync_flop;
            reg                 meta_select;
               always @(negedge reset_in_n or posedge sync_clock_in) 
               begin
                     meta_select <= $urandom;
               end
               always @(negedge reset_in_n or posedge sync_clock_in) 
               begin
                  ...
                  begin
                     meta_sync_flop  <= upstream_reset_in_n;
                  end
               end
            always @(negedge reset_in_n or posedge sync_clock_in) 
            begin
               ...
               begin
                     reset_sync_flop <= {reset_sync_flop[NUM_FLOPS-2:0], meta_select ? upstream_reset_in_n : meta_sync_flop};
               end
            end
...
module scan_mux_cdn_sd1000_t16ffc_01_vc176_2xa1xd3xe2y2r (
   input  scan_mode_en_in,  
...
`timescale 1ns/1fs
module usb2_pma_top_cdn_sd1000_t16ffc_01_vc176_2xa1xd3xe2y2r(
...
wire drive_pg;
parameter hstx_clk_delay = 0.4;
parameter hstx_rstn_delay = 0.2;
parameter hstx_drive_delay = 1;
...
parameter ted_sq_assert_delay = 2;
parameter ted_sq_deassert_delay = 6;
always_comb begin
 if(ted_pg) begin
...
always@(*) begin
 if(ted_pg) begin
  if(bias_ok_ted) begin
  if((clipped_dn ^ clipped_dp))
   o_ted_squelch_ana <= #ted_sq_assert_delay 1'b0;
 else
   o_ted_squelch_ana <= #ted_sq_deassert_delay 1'b1;
   o_ded_ana <= 1'b0;
  end
  else begin
   o_ted_squelch_ana <= 1'b1;
  end             
 end
 else begin
  o_ted_squelch_ana <= 1'bz;
 end
end
