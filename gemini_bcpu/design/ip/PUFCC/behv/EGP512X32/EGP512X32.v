...
EGP512X32_PWR EGP512X32_PWR (
	.PENVDD2_VDD (PENVDD2_VDDi), .PDSTB (PDSTBi), .CTRL_PINS (CTRL_PINSi), .power_ready (power_ready)
);
/**************************************************************/
/*  [END] POWER_SEQ                                           */
/*  CONNECT_MODULE                                            */
...
module EGP512X32_PWR (
	PENVDD2_VDD, PDSTB, CTRL_PINS, power_ready
	);
	input PENVDD2_VDD;
	input PDSTB;
	input CTRL_PINS;
...
wire power_ready_PENVDD2_VDD;
reg pwr_vio_PENVDD2_VDD;
initial pwr_vio_PENVDD2_VDD = 0;
event event_vio_PENVDD2_VDD;
always @ ( event_vio_PENVDD2_VDD )
    TOGGLE_TASK (pwr_vio_PENVDD2_VDD);

pwr_up_step penvdd2_vdd (
        .pwr_pin        (power_on_flag),
        .target         (PENVDD2_VDD),
        .timing_vio     (pwr_vio_PENVDD2_VDD),
        .pwr_seq_step   (power_ready_PENVDD2_VDD));
// 1 target rising - SIGNAL
always @ ( posedge power_ready_PENVDD2_VDD )
begin
        if ( power_ready_PENVDD2_VDD === 1 && PDSTB !== 0 )
        begin
            $display ( "%t ", $realtime, " @@ EMTC_Error : %s cannot set High when %s isn't Low.", "PENVDD2_VDD", "PDSTB");
            -> event_vio_PENVDD2_VDD;
        end
        if ( power_ready_PENVDD2_VDD === 1 && CTRL_PINS !== 0 )
        begin
            $display ( "%t ", $realtime, " @@ EMTC_Error : %s cannot set High when %s isn't Low.", "PENVDD2_VDD", "CTRL_PINS");
            -> event_vio_PENVDD2_VDD;
        end
...
always @ ( negedge PENVDD2_VDD )
begin
        if ( power_ready_PENVDD2_VDD === 1 && PDSTB !== 0 )
        begin
            $display ( "%t ", $realtime, " @@ EMTC_Error : %s cannot set Low when %s isn't Low.", "PENVDD2_VDD", "PDSTB");
            -> event_vio_PENVDD2_VDD;
        end
        if ( power_ready_PENVDD2_VDD === 1 && CTRL_PINS !== 0 )
        begin
            $display ( "%t ", $realtime, " @@ EMTC_Error : %s cannot set Low when %s isn't Low.", "PENVDD2_VDD", "CTRL_PINS");
            -> event_vio_PENVDD2_VDD;
        end
...
pwr_up_step pdstb (
        .pwr_pin        (power_ready_PENVDD2_VDD),
        .target         (PDSTB),
...
specify
    $setuphold ( posedge PDSTB, posedge PENVDD2_VDD &&& power_on_flag, `EMTC_Timing_Tpens_Min , 0, notify_tpens_min );
    $setuphold ( negedge PDSTB, negedge PENVDD2_VDD &&& power_on_flag, 0, `EMTC_Timing_Tpenh_Min, notify_tpenh_min );
endspecify
...
begin
    if ( power_on_flag === 1 && PENVDD2_VDD !== 1 )
    begin
        $display ( "%t ", $realtime, " @@ EMTC_Error : %s cannot set High when %s isn't High.","PDSTB", "PENVDD2_VDD");
        -> event_vio_power_on_flag;
    end
...
begin
    ...
    if ( power_on_flag === 1 && PENVDD2_VDD !== 1 )
    begin
        $display ( "%t ", $realtime, " @@ EMTC_Error : %s cannot set High when %s isn't High.","CTRL_PINS", "PENVDD2_VDD");
        -> event_vio_power_on_flag;
    end
...
// virtual power
    ...
    begin
            if ( power_on_flag === 1 && PENVDD2_VDD !== 0 )
            begin
                $display ( "%t ", $realtime, " @@ EMTC_Error : %s should be set Low.\n If this is initial reset issue ( input pin from unknwon to Low status ). Please modify \"%m.power_on_time\" after reset time. You can use command \"defparam\" to modify it in top module. ", "PENVDD2_VDD");
                -> event_vio_PENVDD2_VDD;
            end
