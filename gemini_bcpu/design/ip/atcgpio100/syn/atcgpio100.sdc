...
# Constraint the following inputs though they are asynchronous
set_input_delay -max [expr $pclk_period * 0.7] -clock PCLK [get_ports extclk]
set_input_delay [expr $pclk_period * 0.7] -max -clock PCLK  \
	[remove_from_collection [get_ports gpio_in*] [get_ports gpio_intr]]

# Set Output Delays
set_output_delay -max [expr $pclk_period * 0.7] -clock PCLK [get_ports prdata*]
set_output_delay -max [expr $pclk_period * 0.7] -clock PCLK [get_ports gpio_intr]

# Constraint the following outputs though they are asynchronous
set_output_delay -max [expr $pclk_period * 0.7] -clock PCLK [get_ports gpio_out*]
set_output_delay -max [expr $pclk_period * 0.7] -clock PCLK [get_ports gpio_oe*]
set_output_delay -max [expr $pclk_period * 0.7] -clock PCLK [get_ports gpio_pullup*]
set_output_delay -max [expr $pclk_period * 0.7] -clock PCLK [get_ports gpio_pulldown*]
