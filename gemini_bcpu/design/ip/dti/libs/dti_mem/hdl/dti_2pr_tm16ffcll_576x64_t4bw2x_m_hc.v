...
module dti_2pr_tm16ffcll_576x64_t4bw2x_m_hc (VDD, VSS, DO, A_R, A_W, T_A_R, T_A_W, DI, T_DI, 
       T_BE_N, CE_N_R, CE_N_W, T_CE_N_R, T_CE_N_W, BWE_N, T_BWE_N, T_RWM_R, T_RWM_W, CLK_R, CLK_W);
inout  VDD;
inout  VSS;
output [63:0] DO;               // Data Output
...
input  [63:0] T_DI;             // Bist Data Input
...
input  [63:0] BWE_N;            // Bit Write Enable --- Active Low
input  [63:0] T_BWE_N;          // Bist Bit Write Enable --- Active Low
...
wire [63:0] DI_INT;                 // Data Input
wire [63:0] BWE_N_INT;            // Bit Write Enable --- Active Low
wire [63:0] BYWE_N_INT;           // Byte Write Enable --- Active Low
...
wire [63:0] T_DI_INT;               // Bist Data Input
wire [63:0] T_BWE_N_INT;            // Bist Bit Write Enable --- Active Low
...
reg   CE_N_R_R;
reg   CE_N_R_W;
reg   [63:0] memArray [575:0];
integer  j;
integer  k;
integer  i;
reg   [63:0] tmpDataIn_R;
reg   [63:0] We_R;
reg   coll_sig;
reg   read_flag;
reg   inWrite;
...
`ifdef ISOLATION
  ...
  assign DI_INT            = (ISOL_N===1'b0) ? {64{1'b0}}  : ( (ISOL_N===1'b1) ? DI           : 'bx );
  `ifdef BIT_WRITE
    assign BWE_N_INT       = (ISOL_N===1'b0) ? {64{1'b1}}  : ( (ISOL_N===1'b1) ? BWE_N        : 'bx );
  `else
    `ifdef BYTE_WRITE
    ...
    `else
      assign BWE_N_INT       = (ISOL_N===1'b0) ? {64{1'b1}}  : ( (ISOL_N===1'b1) ? 'b0          : 'bx );
    `endif
  `endif
  `ifdef OUTPUT_ENABLE
  ...
  `ifdef BIST_TEST
    ...
    assign T_DI_INT        = (ISOL_N===1'b0) ? {64{1'b0}}  : ( (ISOL_N===1'b1) ? T_DI         : 'bx );
    `ifdef BIT_WRITE
      assign T_BWE_N_INT     = (ISOL_N===1'b0) ? {64{1'b1}}  : ( (ISOL_N===1'b1) ? T_BWE_N        : 'bx );
    `else
      `ifdef BYTE_WRITE
      ...
      `else
        assign T_BWE_N_INT       = (ISOL_N===1'b0) ? {64{1'b1}}  : ( (ISOL_N===1'b1) ? 'b0          : 'bx );
      `endif
    `endif
    `ifdef OUTPUT_ENABLE
  ...
  `else
    ...
    assign T_DI_INT        = (ISOL_N===1'b0) ? {64{1'b0}}  : ( (ISOL_N===1'b1) ? 'b0          : 'bx );
  ...
  if(COREPWS_N_INT && P_PWS_N_INT) begin
    if(ok_read) begin 
      ...
      if((!CE_N_R_R && (^Address_R_R === 1'bx)) || (CE_N_R_R === 1'bx)) begin 
        ...
        Dout = (COREPWS_N_INT && P_PWS_N_INT) ? memArray[Address_R_R]: 64'bx;
...
assign Dout_R_INT = ((COREPWS_N_INT===1'bx) || (P_PWS_N_INT===1'bx)) ? 64'bx : (((COREPWS_N_INT===1'b0) || (P_PWS_N_INT===1'b0)) ? 64'b0 : Dout);
...
specify
 specparam
  period_param_0 = 0.3856,
  period_param_1 = 0.4045,
  period_param_2 = 0.4254,
  period_param_3 = 0.4444,
  period_param_4 = 0.4848,
  period_param_5 = 0.5033,
  period_param_6 = 0.5247,
  period_param_7 = 0.5436,
  trise_CLK_R_DO_worst = 0.3715, 
  tfall_CLK_R_DO_worst = 0.3703, 
  trise_CLK_R_DO_worst_0 = 0.3126, 
  tfall_CLK_R_DO_worst_0 = 0.3115, 
  trise_CLK_R_DO_worst_1 = 0.3315, 
  tfall_CLK_R_DO_worst_1 = 0.3304, 
  trise_CLK_R_DO_worst_2 = 0.3524, 
  tfall_CLK_R_DO_worst_2 = 0.3513, 
  trise_CLK_R_DO_worst_3 = 0.3715, 
  tfall_CLK_R_DO_worst_3 = 0.3703, 
  trise_CLK_R_DO_worst_4 = 0.4118, 
  tfall_CLK_R_DO_worst_4 = 0.4107, 
  trise_CLK_R_DO_worst_5 = 0.4304, 
  tfall_CLK_R_DO_worst_5 = 0.4293, 
  trise_CLK_R_DO_worst_6 = 0.4517, 
  tfall_CLK_R_DO_worst_6 = 0.4506, 
  trise_CLK_R_DO_worst_7 = 0.4707, 
  tfall_CLK_R_DO_worst_7 = 0.4696, 
  t_T_RWM_R_setup_worst = 0.0930,
  t_T_RWM_W_setup_worst = 0.0930,
  t_T_RWM_R_hold_worst  = 0.4444,
  t_T_RWM_W_hold_worst  = 0.4444,
  t_A_R_setup_worst = 0.1093,
  t_A_W_setup_worst = 0.1093,
  t_A_R_hold_worst  = 0.0341,
  t_A_W_hold_worst  = 0.0341,
  t_DI_setup_worst = 0.0202,
  t_DI_hold_worst  = 0.1528,
  t_BWE_N_setup_worst = 0.0202,
  t_BWE_N_hold_worst  = 0.1528,
  t_T_BE_N_setup_worst = 2.0680,
  t_T_BE_N_hold_worst  = 0.4444,
  t_T_A_R_setup_worst = 0.1457,
  t_T_A_W_setup_worst = 0.1457,
  t_T_A_R_hold_worst  = 0.0093,
  t_T_A_W_hold_worst  = 0.0093,
  t_T_DI_setup_worst = 0.0279,
  t_T_DI_hold_worst  = 0.1460,
  t_T_CE_N_R_setup_worst = 0.1717,
  t_T_CE_N_W_setup_worst = 0.1717,
  t_T_CE_N_R_hold_worst  = 0.0425,
  t_T_CE_N_W_hold_worst  = 0.0425,
  t_T_BWE_N_setup_worst = 0.0279,
  t_T_BWE_N_hold_worst  = 0.1460,
