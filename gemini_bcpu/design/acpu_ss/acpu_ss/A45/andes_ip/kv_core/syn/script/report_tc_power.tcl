...
for {set itr 1} {$itr <= $last_itr} {incr itr} {
	...
	if {$synthesis_upf} {
		...
		if {${switch_lib_tc} ne ""} {
			append link_library " ${switch_lib_tc}.db"
			append target_library " ${switch_lib_tc}.db"
		}	
		if {${iso_lvl_lib_tc} ne ""} {
			append link_library " ${iso_lvl_lib_tc}.db"
			append target_library " ${iso_lvl_lib_tc}.db"
		}	
	}
	...
	read_file -format sverilog -netlist ./netlist/${root_design}${itr}.vg
