# ============================
if {[llength [get_pins -quiet ae350_apb_subsystem/u_spi*/u_spi_reg/spi_r_clk_inferred_i_*/O]]} {
	# Vivado has difficulties at deciding the clock edge used so let's guide it.
	if {$spi_mode_0 == $spi_mode_1} {
		set_clock_sense -positive [get_pins ae350_apb_subsystem/u_spi*/u_spi_reg/spi_r_clk_inferred_i_*/O]
	} else {
		set_clock_sense -negative [get_pins ae350_apb_subsystem/u_spi*/u_spi_reg/spi_r_clk_inferred_i_*/O]
	}
}

# Fix clock region issue
set_property CLOCK_DEDICATED_ROUTE FALSE [get_nets [get_property PARENT [get_nets ae350_aopd/sample_ae350_clkgen/ae350_fpga_clkgen/clk_out5]]]
set_property CLOCK_DEDICATED_ROUTE FALSE [get_nets [get_property PARENT [get_nets ae350_aopd/sample_ae350_clkgen/ae350_fpga_clkgen/clk_out6]]]
...
if {[llength [get_ports -quiet X_secure_mode*]]} {
	set_property	IOSTANDARD	$bank16_iostandard	[get_ports {X_secure_mode[0]}]
	set_property	IOSTANDARD	$bank16_iostandard	[get_ports {X_secure_mode[1]}]
	set_property	PACKAGE_PIN	A8			[get_ports {X_secure_mode[0]}]
	set_property	PACKAGE_PIN	C9			[get_ports {X_secure_mode[1]}]
}
