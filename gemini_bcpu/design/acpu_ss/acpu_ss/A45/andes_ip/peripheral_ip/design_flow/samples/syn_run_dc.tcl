...
set standard_cell_list [glob -nocomplain -types f -directory $tech_lib_path ${tech_lib}.db] 
set memory_lib_list [glob -nocomplain -types f -directory $memory_lib_path *_${mem_cond}.db]
set lib_list "$standard_cell_list $memory_lib_list"
if {[info exists pad_lib]} {
	set lib_list [concat $lib_list ${pad_lib}.db]
}
set target_library "$standard_cell_list"
...
concat "config.inc" $hdl_file_list

analyze -define $syn_define -format sverilog $hdl_file_list
...
if {[info exists load_gck_cell]} {
	...
	if {[gck_autogen "gck_autogen.v"]} {
		...
		analyze -define $syn_define -format sverilog "gck_autogen.v"
	} else {
		puts "read_design.tcl: WARNING: fail to generate gck_autogen.v ."
		puts "=== load gck.v ==="
		analyze -define $syn_define -format sverilog "$NDS_HOME/andes_ip/macro/gck.v"
	}
}
...
if {[info exists param]} {
	elaborate $design_name -parameters $param > elaborate.log
} else {
	elaborate $design_name > elaborate.log
}
...
if {"$syn_effort" == ""} {
	compile_ultra -no_seq_output_inversion -no_autoungroup > compile.log
} else {
	compile_ultra -timing_high_effort_script -no_seq_output_inversion -no_autoungroup > compile.log
}
