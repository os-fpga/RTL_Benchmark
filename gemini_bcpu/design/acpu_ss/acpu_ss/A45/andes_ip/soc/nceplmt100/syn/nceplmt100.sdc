...
create_clock -name MTIME_CLK -period $pclk_period \
   -waveform [list 0 [expr $pclk_period / 2.0]] [get_ports mtime_clk]
...
set_input_delay -max [expr $bus_io_delay] -clock HCLK [get_ports hburst*]
set_input_delay -max [expr $bus_io_delay] -clock HCLK [get_ports hready]
set_input_delay -max [expr $bus_io_delay] -clock HCLK [get_ports resetn]
...
set_input_delay -max [expr $bus_io_delay] -clock MTIME_CLK [get_ports por_rstn]
...
set_output_delay -max [expr $bus_io_delay] -clock HCLK [get_ports mtip]
...
# -------------------------------------------------------------
# Specify False Paths
set_false_path -from [get_clocks "HCLK"] -to [get_clocks "MTIME_CLK"]
set_false_path -from [get_clocks "MTIME_CLK"] -to [get_clocks "HCLK"]
