...
architecture CU_STRUCT of CPU_CU is
	...
	SIGNAL dwait_valid_x	: std_logic;
...
begin
	...
	begin
		...
		if ((nreset_in /= nreset_value) and (nreset_v > UNRESET_IU)) then
			...
			if ((dwait_in = '1') and (C_dvalid = '1') and (E_c /= IWAIT_E)) then
				E_x		<=	DWAIT_E;
			elsif (iwait_in = '1') then
				E_x		<=	IWAIT_E;
			elsif ((int_in = '1') and (S_c = NORMAL) and (int_stop_c = '0')) then	-- WAIT 2 CLOCKS AFTER RTI TO AVOID LOCKS
				E_x		<=	INT_E;
			else
				E_x		<=	NONE_E;
			end if;
	...
	begin
		...
		if (nreset_v < UNRESET_CU) then -- (reset or starting)
			...
			if (E_x = DWAIT_E)	then		--	(dwait_in = '1')
				idata_x		<=	idata_c; 
				if (E_c = IWAIT_E) then
					skip_x	<= skip_c;
				else
				...
				ndre_x 		<= '1';
				dwait_valid_x	<= '1';
				int_start_x	<=	int_start_c;
				int_stop_x	<=	int_stop_c;
				S_x 		<= 	S_c;
			elsif (E_x = IWAIT_E)	then		--	(iwait_in = '1')
				dwait_valid_x	<= '0';
				if (E_c = IWAIT_E) then
					skip_x	<= skip_c;
				else
