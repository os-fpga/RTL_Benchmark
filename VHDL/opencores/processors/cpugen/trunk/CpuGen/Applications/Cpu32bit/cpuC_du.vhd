...
architecture DUC_STRUCT of CPUC_DU is
...
begin
	...
	DU_ALU: process(nreset_in, nreset_v, valid, code, code_c, class_cu,
				acc_i, skip_i,
				class_du, acc_c, data_x, data_is_int, data_in_int, acc, mul, skip_l)
	begin
			...
			case class_cu is
				...
				when TC_OTH 	=>
					case class_du is
						...
						-- SKV
						when TD_SKV =>
							if (acc_c(2)(DATA_LEN) = '1') then skip_l <= '1';
							else  skip_l <= '0';
							end if;
							code	<=	SKV_I;
						-- SKNV
						when TD_SKNV =>
							if (acc_c(2)(DATA_LEN) = '0') then skip_l <= '1';
							else  skip_l <= '0';
							end if;
							code	<=	SKNV_I;
						-- OTHERS
				...
				when TC_LDA | TC_DAT	=>
					...
					case class_du is
						...
						-- ADDS
						when TD_ADDS =>
							acc(0)	<=	(('0' & signed(acc_c(0)((DATA_LEN - 1) downto 0))) + ('0' & signed(data_x)));
							if ((acc_c(0)(DATA_LEN - 1) /= data_x(DATA_LEN - 1)) or ((acc_c(0)(DATA_LEN - 1) = acc(0)(DATA_LEN - 1)))) then
								acc(2)(DATA_LEN) <= '0';	-- NO OVERFLOW
							else
								acc(2)(DATA_LEN) <= '1';	-- OVERFLOW
							end if; 
							if (class_cu  = TC_DAT) then
								code	<=	ADDSI_I;
							else
						...
						-- MULS
						when TD_MULS =>
							mul	<=	((signed(acc_c(0)((DATA_LEN - 1) downto 0))) * (signed(data_x)));
							acc(1) <=	'0' & mul((2 * DATA_LEN - 1) downto DATA_LEN);
							acc(0) <=	'0' & mul((DATA_LEN - 1) downto 0);
							if (class_cu  = TC_DAT) then
								code	<=	MULSI_I;
							else
								code	<=	MULS_I;
						...
						-- MUL
						when TD_MUL =>
							mul	<=	((unsigned(acc_c(0)((DATA_LEN - 1) downto 0))) * (unsigned(data_x)));
							acc(1) <=	'0' & mul((2 * DATA_LEN - 1) downto DATA_LEN);
							acc(0) <=	'0' & mul((DATA_LEN - 1) downto 0);
							if (class_cu  = TC_DAT) then
								code	<=	MULI_I;
						...
						when TD_SHLN =>
							acc(0)	<=	('0' & Conv_Std_logic_vector(SHL(unsigned(acc_c(0)),unsigned(data_x)),DATA_LEN));
							if (class_cu  = TC_DAT) then
								code	<=	SHLNI_I;
							else
								code	<=	SHLN_I;
						...
						when TD_SHRN =>
							acc(0)	<=	('0' & Conv_Std_logic_vector(SHR(unsigned(acc_c(0)),unsigned(data_x)),DATA_LEN));
							if (class_cu  = TC_DAT) then
								code	<=	SHRNI_I;
							else
								code	<=	SHRN_I;
